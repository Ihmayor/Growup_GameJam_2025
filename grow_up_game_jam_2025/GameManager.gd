...
func calculate_plant_total():
	...
	for planted_slot: Slot in array_of_slots_taken:
		...
		if !plant_neighbour_dictionary.has(plant_id):
			print(plant_neighbour_dictionary)
		...
		if planted_slot.location.x > 0:
			found_slot = total_slots.filter(func(n): return n.location == Vector2(planted_slot.location.x - 1, planted_slot.location.y)).get(0)
		...
		if left_slot && !plant_neighbour_dictionary[plant_id]["plant_ids"].has(left_slot.plant_node.get_instance_id()):
		...
		if right_slot && !plant_neighbour_dictionary[plant_id]["plant_ids"].has(right_slot.plant_node.get_instance_id()):
